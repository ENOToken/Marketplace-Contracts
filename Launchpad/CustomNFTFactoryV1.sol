...
contract CustomNFTFactoryV1 is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function createNFT(NFTDataStructures.NFTCreateParams memory params) 
        external 
        whenNotPaused 
        nonReentrant 
        returns (address) 
    {
        ...
        if (params.maxMintsPerWallet == 0) revert InvalidMaxMints();
    ...
    function getCreatedNFTsPaginated(uint256 offset, uint256 limit) 
        external 
        view 
    ...
    function isValidNFTContract(address nftAddress) external view returns (bool) {
